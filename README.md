...
3. **환경 설정**:
   ...
   ```
   > [!TIP]
   > 파일을 직접 만들기 어렵다면 안티그래비티에게 **"내 API 키 `AIza...`로 .env 파일 만들어줘"**라고 말하세요.
...
- **환경 설정 (.env 파일 생성)**:
  직접 파일을 만들 필요 없이, 채팅창에 자신의 API 키를 넣어서 말만 하세요.
  > "내 Gemini API 키는 `AIza...`이야. 이걸로 .env 파일 만들어서 저장해줘."
...
- **주소**: [http://localhost:3095](http://localhost:3095)
